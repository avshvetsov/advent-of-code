...
public class Day22 {
    ...
    public long partTwoAnton(List<String> inputList) {
        ...
        Map<Integer, Integer> fallCountMap = new HashMap<>();
        for (Map.Entry<Integer, Pair<Set<Integer>, Set<Integer>>> entry : aboveUnderMap.entrySet()) {
            if (!entry.getValue().getSecond().isEmpty()) {
                Set<Integer> fallenSet = new HashSet<>();
                fallenSet.add(entry.getKey());
                fallCountMap.put(entry.getKey(), countFallen(aboveUnderMap, fallenSet, entry.getValue().getSecond()));
            } else {
                fallCountMap.put(entry.getKey(), 0);
            }
        }

        return fallCountMap.values().stream().mapToInt(integer -> integer).sum();
    }

    private Integer countFallen(Map<Integer, Pair<Set<Integer>, Set<Integer>>> aboveUnderMap, Set<Integer> fallenSet, Set<Integer> potentialFallen) {
        int prevFallenSetSize = -1;
        while (!potentialFallen.isEmpty() && fallenSet.size() != prevFallenSetSize) {
            prevFallenSetSize = fallenSet.size();
            Set<Integer> newPotentialFallen = new HashSet<>();
            for (Integer potential : potentialFallen) {
                Pair<Set<Integer>, Set<Integer>> aboveUnder = aboveUnderMap.get(potential);
                Set<Integer> above = aboveUnder.getFirst();
                Set<Integer> under = aboveUnder.getSecond();
                if (fallenSet.containsAll(above)) {
                    fallenSet.add(potential);
                    newPotentialFallen.addAll(under);
                } else {
                    newPotentialFallen.add(potential);
                }
            }
            potentialFallen = newPotentialFallen;
        }
        return fallenSet.size() - 1;
    }
